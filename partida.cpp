...
Partida::Partida() : en_ejecucion(false), monitor_jugadores(new MonitorJugadores()), cola(new Queue<std::string>()), escenario(200, 200, monitor_jugadores)
{
...
void Partida::run()
{
    ...
    while (en_ejecucion)
    {
        ...
        std::string comando;
        if (cola->try_pop(comando))
        {
            std::istringstream iss(comando);
            std::string palabra;
            iss >> palabra;
            if (palabra == "DESCONEXION")
            {
                monitor_jugadores->avisar_desconexion();
                iss >> id_desconectado;
            }
            else
            {
                escenario.procesar(comando);
            }
        }
