...
GranadaVerde::GranadaVerde(bool direccion, int angulo, int segundos, int potencia)
{
...
GranadaVerde::GranadaVerde(Mundo *mundo, b2Body *cuerpo, int segundos)
{
    tiempo_hasta_explotar = segundos;
    esperando_a_explotar = true;
    this->mundo = mundo;
    body = cuerpo;
    danio = configuracion.getDanioGranadaVerde();
    radio = configuracion.getRadioGranadaVerde();
    velocidad_minima = 1.2f;
}

int GranadaVerde::disparar(Mundo *mundo, b2Body *disparador)
{
    this->mundo = mundo;
    ...
    Objeto *granada_santa = new GranadaVerde(mundo, body, tiempo_hasta_explotar);
    mundo->agregar_objeto(granada_santa);
    return 0;
}

PosicionLanzable GranadaVerde::conseguir_posicion()
{
    // contacto();
    ...
    return PosicionLanzable(GREEN_GRENADE, x, y, 0, angulo, is_dead);
}

void GranadaVerde::explotar_regresiva()
{
