...
ObjetoDTO::ObjetoDTO(int x, int y, bool direccion, int angulo, int tipo, bool explosion) : x(x), y(y), direccion(direccion), angulo(angulo), tipo(tipo), explosion(explosion)
{
}

void ObjetoDTO::mostrar() const { std::cout << "Jugador id " << id << " en X: " << x << " Y: " << y << " Angulo:" << angulo << std::endl; }
